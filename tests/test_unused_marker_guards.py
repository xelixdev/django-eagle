...
class TestMarkerGuardsOutsideActiveRequest:
    def test_marking_functions_are_inert_outside_a_request(self):
        eagle = EagleFactory()
    ...
    def test_mark_prefetched_skips_instance_without_initialized_state(self):
        eagle = EagleFactory()
        unused.begin_request()
        unused.mark_prefetched([eagle], "previous_locations")
        unused.end_request()

    def test_second_init_state_call_does_not_overwrite_location(self):
        eagle = EagleFactory()
        unused.begin_request()
